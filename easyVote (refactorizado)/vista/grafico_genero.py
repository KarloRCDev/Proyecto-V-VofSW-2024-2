...
class GraficoGenero:
    ...
    def crear_grafico(self):
        ...
        ax1.barh(self.candidatosM, self.votosM)
        for i, v in enumerate(self.votosM):
            # Ajustar posición de los textos
            ax1.text(v, i, str(v), ha='left', va='center')
        ax1.set_title("Masculino")

        # Crear eje de barras para el segundo gráfico
        ax2 = fig.add_subplot(122)
        # Usar barh para barras horizontales
        ax2.barh(self.candidatosF, self.votosF, color='red')

        for i, v in enumerate(self.votosF):
            # Ajustar posición de los textos
            ax2.text(v, i, str(v), ha='left', va='center')
